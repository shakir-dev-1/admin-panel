...
export class PaymentsService {
  ...
  async createPaymentIntent(
    ...
    amount: number,
    invoiceId: string,
    clientId?: string, // Add optional clientId
  ) {
    console.log(
    ...
    return this.prisma.$transaction(async (prisma) => {
      ...
      if (clientId) {
        ...
        if (!client) {
          ...
          console.log(
            `[createPaymentIntent] Created test client: ${client.id}`,
          );
        }
      }
      ...
      // Create Stripe PaymentIntent with automatic confirmation
      ...
      // Create invoice with the appointment
